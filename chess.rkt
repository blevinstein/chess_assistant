...
(define bg-white (esc 48 5 246)) ; light gray
(define bg-black (esc 48 5 242)) ; dark gray
...
(define (render-square background square)
...
(define (print-grid grid)
  (for ([i (in-range 8)])
    (for ([j (in-range 8)])
      ...
      (render-square (if (equal? (modulo (+ i j) 2) 0) 'black 'white) square))
    (displayln reset)))
